...
public class ExampleOfUse {
    public static void main(String[] args) throws Exception {
        ...
        // lista de threads
        Collection<CalculatePrimes> threads = new ArrayList<>();
        ...
        for (int i = 1; i <= number_threads; i++) {
            ...
            int trab = Math.round(work / number_threads);

            // calcula o valor inicial e final do intervalo de cada thread
            int end = trab * i;
            int start = (end - trab) + 1;

            // cria a thread com a classe CalculePrimes que estende da classe Thread
            CalculatePrimes thread = new CalculatePrimes(start, end, primes);
            ...
            for (CalculatePrimes cp : threads) {
                cp.start();
            }
